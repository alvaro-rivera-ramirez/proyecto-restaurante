const conn = require("../config/bd");
  ...
  const catMes = async (arrayfecha) => {
    const users = await conn.query(`select cat.nom_categoria,sum(prod.precio_u_prod*det.cantidad_det) as sumDet from categoria as cat inner join producto as prod on cat.id_categoria=prod.id_categoria 
