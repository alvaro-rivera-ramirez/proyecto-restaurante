const addDetail=(idprod,nomprod,countProd,subtotal,observation)=>{
    ...
    <div class="col-md-3">
        ${subtotal.toLocaleString('es-PE', { style: 'currency', currency: 'PEN' })}
    </div>
...
const renderInfoOrder=(orderInfo,tables)=>{
    ...
    totalOrder.innerHTML=`${orderInfo.total.toLocaleString('es-PE', { style: 'currency', currency: 'PEN' })}`
}
const fetchOrder=async ()=>{
    ...
    try {
        ...
        detailsOrder.forEach(detail => {
            addDetail(detail.id_prod,detail.nom_prod,detail.cantidad_det,detail.subtotal,detail.descripcion_det);
        });
